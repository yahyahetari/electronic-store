...
export default function ProductBox({ _id, title, images, variants, slug, ratings }) {
  ...
  const getStockStatus = () => {
    if (totalStock === 0) {
      return { status: 'out', label: 'نفذت الكمية', color: 'bg-red-500', textColor: 'text-red-500' };
    } else if (totalStock <= 5) {
      return { status: 'low', label: `${totalStock} قطع متبقية`, color: 'bg-orange-500', textColor: 'text-orange-500' };
    } else if (totalStock <= 10) {
      return { status: 'low', label: 'كمية محدودة', color: 'bg-yellow-500', textColor: 'text-yellow-500' };
    }
    return { status: 'available', label: 'متوفر', color: 'bg-green-500', textColor: 'text-green-500' };
  };
  ...
  return (
    <>
      <motion.div
        className="flex flex-col gap-1 border shadow-2xl border-slate-800 rounded-lg h-full relative"
        initial={{ opacity: 0, y: -50 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.5 }}
      >
        {/* Out of Stock Overlay */}
        {isOutOfStock && isImageLoaded && (
          <div className="absolute inset-0 bg-black bg-opacity-60 rounded-lg z-10 flex items-center justify-center">
            <div className="text-center">
              <div className="bg-red-500 text-white px-6 py-3 rounded-lg font-bold text-lg mb-2">
                نفذت الكمية
              </div>
              <p className="text-white text-sm">سيتوفر قريباً</p>
            </div>
          </div>
        )}
          ...
          <>
            <Link href={`/product/${slug}`}>
              <div className="relative overflow-hidden group">
                ...
                {/* Stock Status Badge */}
                {!isOutOfStock && stockStatus.status === 'low' && (
                  <div className="absolute top-3 left-3 z-10">
                    <div className={`${stockStatus.color} text-white text-xs font-bold px-2 py-1 rounded-full shadow-lg animate-pulse`}>
                      {stockStatus.label}
                    </div>
                  </div>
                )}
            ...
            <div className="pr-2 pb-1 text-right flex-grow flex flex-col">
              ...
              {/* Stock Status Text */}
              <p className={`text-xs font-semibold mt-1 ${stockStatus.textColor}`}>
                {stockStatus.label}
              </p>
            </div>
            ...
            <div className="pr-2 flex justify-between items-center mt-auto pb-2">
              ...
              <svg
                ...
                className={`w-14 py-1 rounded-lg p-4 border ml-2 transition-all ${
                  isOutOfStock 
                    ? 'border-gray-400 opacity-50 cursor-not-allowed' 
                    : 'border-black cursor-pointer hover:bg-black hover:fill-white'
                }`}
                viewBox="0 0 576 512"
